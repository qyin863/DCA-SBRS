...
from .functions import pad_zero_for_seq, pad_zero_for_seq_cat, build_seqs, build_seqs_cat, get_seq_from_df, get_seq_from_df_cat
...
class NARMDataset_Cat(Dataset):
    def __init__(self, data, conf):
        ...
        '''
        # self.data is list of [[seqs],[targets]]
        self.data = build_seqs_cat(get_seq_from_df_cat(data, conf), conf['session_len'])
        
    def __getitem__(self, index):
        session_items = self.data[0][index]
        session_categories = self.data[1][index]
        target_item = self.data[2][index]
        target_category = self.data[3][index]
        return session_items, session_categories, target_item, target_category
    ...
    def get_loader(self, args, shuffle=True):
        loader = DataLoader(
            ...
            batch_size=args['batch_size'],
            shuffle=shuffle,
            collate_fn=pad_zero_for_seq_cat
        )
...
class GCEDataset_Cat(Dataset):
    def __init__(self, data, conf, train_len=None):
        data = build_seqs_cat(get_seq_from_df_cat(data, conf), conf['session_len'])
        inputs, mask, max_len = self.handle_data(data[0], train_len)
        self.inputs = np.asarray(inputs)
        cat_inputs, _, _ = self.handle_data(data[1], train_len)
        self.cat_inputs = np.asarray(cat_inputs)
    ...
    def __getitem__(self, index):
        u_input, c_input, mask, target = self.inputs[index], self.cat_inputs[index], self.mask[index], self.targets[index]

        max_n_node = self.max_len
        node = np.unique(u_input)
        cat_node = np.unique(c_input)
        items = node.tolist() + (max_n_node - len(node)) * [0]
        adj = np.zeros((max_n_node, max_n_node))
        for i in np.arange(len(u_input) - 1):
            u = np.where(node == u_input[i])[0][0]
        ...
        alias_inputs = [np.where(node == i)[0][0] for i in u_input]
        c_alias_inputs = [np.where(cat_node == i)[0][0] for i in c_input]
        
        return [torch.tensor(alias_inputs), torch.tensor(c_alias_inputs), torch.tensor(adj), torch.tensor(items),
                torch.tensor(mask), torch.tensor(target), torch.tensor(u_input), torch.tensor(c_input)]
...
class SRGNNDataset_Cat(object):
    def __init__(self, data, conf, shuffle=False, graph=None):
        data = build_seqs_cat(get_seq_from_df_cat(data, conf), conf['session_len'])
        inputs = data[0]
        inputs, mask, len_max = self.data_masks(inputs, [0])
        cat_inputs = data[1]
        cat_inputs, _, _ = self.data_masks(cat_inputs, [0])
        self.inputs = np.asarray(inputs)
        self.cat_inputs = np.asarray(cat_inputs)
        self.mask = np.asarray(mask)
        self.len_max = len_max
        self.targets = np.asarray(data[2])
        self.cat_targets = np.asarray(data[3])
        self.length = len(inputs)
        self.shuffle = shuffle
        self.graph = graph
    ...
    def generate_batch(self, batch_size):
        if self.shuffle:
            ...
            np.random.shuffle(shuffled_arg)
            self.inputs = self.inputs[shuffled_arg]
            self.cat_inputs = self.cat_inputs[shuffled_arg]
            self.mask = self.mask[shuffled_arg]
            self.targets = self.targets[shuffled_arg]
            self.cat_targets = self.cat_targets[shuffled_arg]
        n_batch = int(self.length / batch_size)
        if self.length % batch_size != 0:
            n_batch += 1
        slices = np.split(np.arange(n_batch * batch_size), n_batch)
    ...
    def get_slice(self, i):
        inputs, mask, targets = self.inputs[i], self.mask[i], self.targets[i]
        cat_inputs, cat_targets = self.cat_inputs[i], self.cat_targets[i]
        items, n_node, A, alias_inputs = [], [], [], []
        categories, n_cate, c_alias_inputs = [], [], []
        for u_input in inputs:
            n_node.append(len(np.unique(u_input)))
        max_n_node = np.max(n_node)

        for c_input in cat_inputs:
            n_cate.append(len(np.unique(c_input)))
        max_n_cate = np.max(n_cate)
        
        for (u_input, c_input) in zip(inputs, cat_inputs):
            node, indices = np.unique(u_input, return_index=True)
            c_node, c_indices = np.unique(c_input, return_index=True)
            items.append(node.tolist() + (max_n_node - len(node)) * [0])
            categories.append(c_node.tolist() + (max_n_cate - len(c_node)) * [0])
            u_A = np.zeros((max_n_node, max_n_node))
            for i in np.arange(len(u_input) - 1):
                if u_input[i + 1] == 0:
                    break
            ...
            A.append(u_A)
            alias_inputs.append([np.where(node == i)[0][0] for i in u_input])
            c_alias_inputs.append([np.where(c_node == i)[0][0] for i in c_input])
        return alias_inputs, c_alias_inputs, A, items, categories, mask, targets
