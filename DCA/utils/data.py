...
from ..utils.functions import reindex
...
class Interactions(object):
    ...
    def _load_data(self):
        ...
        if dataset_name == 'ml-100k':
            ...
            df = pd.read_csv( f'{DATA_PATH}retailrocket/events_withCategory.csv', parse_dates=['timestamp'])
            #specify header names
            df.columns = [self.user_key, self.item_key, self.time_key, self.category_key]
            self.df = df
            self.n_days = 7
...
class Categories(object):
    ...
    def _load_data(self):
        ...
        if dataset_name == 'ml-100k':
            ...
            item = pd.read_csv( f'{DATA_PATH}retailrocket/events_withCategory.csv', usecols=[1,3])
            # drop duplicates
            msk = item.duplicated()
            item = item[~msk]
