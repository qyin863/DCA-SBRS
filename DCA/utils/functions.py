...
def build_seqs_cat(seqs, max_len):
    user_seqs, user_cats, targets, cat_targets, sess = [], [], [], [], []
    for seq in seqs:
        s_id = seq[0]
        items = seq[1]
        categories = seq[2]
        ...
        for j in range(tmp_len - 1):
            targets.append([items[j + 1]])
            cat_targets.append([categories[j + 1]])
            user_seq = items[0:j + 1]
            user_cat = categories[0:j + 1]
            user_seqs.append(user_seq)
            user_cats.append(user_cat)
            sess.append(s_id)

        if len(items) > max_len:
            for j in range(max_len, len(items)):
                targets.append([items[j]])
                cat_targets.append([categories[j]])
                user_seq = items[j - max_len + 1:j]
                user_cat = categories[j - max_len + 1:j]
                user_seqs.append(user_seq)
                user_cats.append(user_cat)
                sess.append(s_id)
    
    return user_seqs, user_cats, targets, cat_targets, sess
    
def get_seq_from_df_cat(df, args):
    dic = df[args['session_key']].drop_duplicates() #[args['user_key'], ]
    seq = []
    # u,
    for s in dic.values:
        items = df.query(f'{args["session_key"]} == {s}')[args["item_key"]].tolist()
        categories = df.query(f'{args["session_key"]} == {s}')[args["category_key"]].tolist()
        seq.append([s, items, categories])

    return seq


def pad_zero_for_seq_cat(data):
    '''
    pad sessions to max length
    ...
    data.sort(key=lambda x: len(x[0]), reverse=True)  # x[0][0]
    labels, cat_labels = [], []
    lens = [len(sess) for sess, _,_,_ in data]  # sess[0]
    padded_sess_item = torch.zeros(len(data), max(lens)).long()  # zero-pad
    padded_sess_category = torch.zeros(len(data), max(lens)).long()  # zero-pad
    for i, (sess, cat_sess, label, cat_label) in enumerate(data):
        padded_sess_item[i, :lens[i]] = torch.LongTensor(sess)
        padded_sess_category[i, :lens[i]] = torch.LongTensor(cat_sess)
        labels.append(label)
        cat_labels.append(cat_label)

    padded_sess_item = padded_sess_item.transpose(0, 1)
    padded_sess_category = padded_sess_category.transpose(0, 1)
    
    return padded_sess_item, padded_sess_category, torch.tensor(labels).long(), torch.tensor(cat_labels).long(), lens
