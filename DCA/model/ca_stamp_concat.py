...
class CA_STAMP_CONCAT(nn.Module):
    def __init__(self, n_items, n_categories, params, logger):
        '''

        '''        
        super(CA_STAMP_CONCAT, self).__init__()
        ...
        self.category_embedding = nn.Embedding(self.n_categories, self.embedding_size, padding_idx=0)
        self.w1 = nn.Linear(2*self.embedding_size, self.embedding_size, bias=False)
        self.w2 = nn.Linear(2*self.embedding_size, self.embedding_size, bias=False)
        self.w3 = nn.Linear(2*self.embedding_size, self.embedding_size, bias=False)
    ...
    def count_alpha(self, context, aspect, output):
        ...
        aspect_3dim = aspect.repeat(1, timesteps).view(-1, timesteps, 2*self.embedding_size)
        output_3dim = output.repeat(1, timesteps).view(-1, timesteps, 2*self.embedding_size)
    ...
    def forward(self, seq, seq_cate, lengths):
        batch_size = seq.size(1)
        seq = seq.transpose(0,1)
        seq_cate = seq_cate.transpose(0,1)
        item_seq_emb = self.item_embedding(seq) # [b, seq_len, emb]
        category_embs = self.category_embedding(seq_cate) # B*L*h
#        last_inputs = self.gather_indexes(item_seq_emb, lengths - 1)
        ...
        category_last_click = torch.gather(seq_cate, dim=1, index=item_last_click_index.unsqueeze(1).long()) # [b, 1]
        last_inputs = self.item_embedding(item_last_click.squeeze())# [b, emb]
        category_last_inputs = self.category_embedding(category_last_click.squeeze())
        org_memory = item_seq_emb # [b, seq_len, emb]
        ms = torch.div(torch.sum(org_memory, dim=1), lengths.unsqueeze(1).float())# [b, emb]
        ms_c = torch.div(torch.sum(torch.cat((org_memory, category_embs), dim=-1), dim=1), lengths.unsqueeze(1).float())# [b, emb]
        alpha = self.count_alpha(torch.cat((org_memory,category_embs), dim=-1), torch.cat((last_inputs,category_last_inputs), dim=-1), ms_c) # [b, seq_len]
    ...
    def fit(self, train_loader, validation_loader=None):
        ...
        for epoch in range(1, self.epochs + 1):  
            ...
            for i, (seq, seq_cate, target, target_cate, lens) in enumerate(train_loader):
                ...
                scores = self.forward(seq.to(self.device), seq_cate.to(self.device), lens)
