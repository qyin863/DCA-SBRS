...
class DCA_NARM(nn.Module):
    def __init__(self, n_items, n_categories, item_cate_matrix, params, logger):
        ...
        self.category_embedding = nn.Embedding(self.n_categories, self.hidden_size, padding_idx=0)
    ...
    def forward(self, seq, seq_cate, lengths):
        ...
        category_embs = self.category_embedding(seq_cate).permute(1, 0, 2) # B*L*h
        assert category_embs.shape == (batch_size, gru_out.shape[1], gru_out.shape[2])
        
        last_category = category_embs[torch.LongTensor(range(category_embs.size(0))).to(self.device), torch.LongTensor(lengths-1).to(self.device),]
        
        c_global = ht
        q1 = self.a_1(gru_out.contiguous().view(-1, self.hidden_size)+category_embs.contiguous().view(-1, self.hidden_size)).view(gru_out.size())
        q2 = self.a_2(ht+last_category)
