...
class DCA_GCSAN(Module):
    def __init__(self, n_node, n_cate, item_cate_matrix, conf, logger):
        '''
        ...
        '''    
        super(DCA_GCSAN, self).__init__()
        self.hidden_size = conf['item_embedding_dim']  # this hidden size is item embedding in SRGNN
        self.n_node = n_node # with padding
        self.n_cate = n_cate
        self.item_cate_matrix = item_cate_matrix[1:, 1:]
        ...
        self.cat_embedding = nn.Embedding(self.n_cate, self.hidden_size, padding_idx=0)
    ...
    def compute_scores(self, hidden, seq_cat, mask, residual=True):
        ...
        seq_cat *=  ~timeline_mask.unsqueeze(-1)
        
        for i in range(self.blocks):
            # Single Attention
            attn_output = attn_output.transpose(0,1)
            Q = self.attention_layernorms[i](attn_output+seq_cat.transpose(0,1))
            mha_outputs, _ = self.attention_layers[i](Q, attn_output+seq_cat.transpose(0,1), attn_output, key_padding_mask=timeline_mask)
    ...
    def _forward(self, i, data):
        alias_inputs, c_alias_inputs, A, items, categories, mask, targets = data.get_slice(i)
        alias_inputs = torch.Tensor(alias_inputs).long().to(self.device)
        c_alias_inputs = torch.Tensor(c_alias_inputs).long().to(self.device)
        items = torch.Tensor(items).long().to(self.device)
        categories = torch.Tensor(categories).long().to(self.device)
        ...
        cat_embs = self.cat_embedding(categories)
        get_c = lambda index: cat_embs[index][c_alias_inputs[index]]
        seq_cat = torch.stack([get_c(i) for i in torch.arange(len(c_alias_inputs)).long()])
        
        return targets, self.compute_scores(seq_hidden, seq_cat, mask)
    ...
    def fit(self, train_data, validation_data=None):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for i, j in zip(slices, np.arange(len(slices))):
                ...
                loss_ce = self.loss_function(scores, (targets - 1).squeeze())
                
                category_scores_consideringNum = self.sf(scores).matmul(self.item_cate_matrix.to(self.device))
                Entropy_pre = Categorical(probs = category_scores_consideringNum).entropy()
