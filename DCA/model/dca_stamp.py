...
class DCA_STAMP(nn.Module):
    def __init__(self, n_items, n_categories, item_cate_matrix, params, logger):
        '''

        '''        
        super(DCA_STAMP, self).__init__()
...
#        last_inputs = self.gather_indexes(item_seq_emb, lengths - 1)
        ...
        ms_c = torch.div(torch.sum(org_memory+category_embs, dim=1), lengths.unsqueeze(1).float())# [b, emb]
        alpha = self.count_alpha(org_memory+category_embs, last_inputs+category_last_inputs, ms_c) # [b, seq_len]
