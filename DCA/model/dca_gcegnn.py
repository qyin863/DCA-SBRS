...
class DCA_CombineGraph(Module):
    def __init__(self, conf, num_node, num_category, item_cate_matrix, adj_all, num, logger):
    # conf is model_conf
        super(DCA_CombineGraph, self).__init__()
        ...
        self.cat_embedding = nn.Embedding(num_category, self.dim)
        self.pos_embedding = nn.Embedding(200, self.dim)

        # Parameters
        self.w_1 = nn.Parameter(torch.Tensor(3 * self.dim, self.dim))
        self.w_2 = nn.Parameter(torch.Tensor(self.dim, 1))
    ...
    def compute_scores(self, hidden, seq_cat, mask):
        ...
        hs = torch.sum((hidden+seq_cat) * mask, -2) / torch.sum(mask, 1)
        hs = hs.unsqueeze(-2).repeat(1, len, 1)
        nh = torch.matmul(torch.cat([seq_cat, pos_emb, hidden], -1), self.w_1)
        nh = torch.tanh(nh)
    ...
    def _forward(self, data):
        alias_inputs, c_alias_inputs, adj, items, mask, targets, inputs, cat_inputs = data
        alias_inputs = alias_inputs.long().to(self.device)
        c_alias_inputs = c_alias_inputs.long().to(self.device)
        ...
        cat_inputs = cat_inputs.long().to(self.device)
        
        hidden = self.forward(items, adj, mask, inputs)
        get = lambda index: hidden[index][alias_inputs[index]]
        seq_hidden = torch.stack([get(i) for i in torch.arange(len(alias_inputs)).long()])
        
        cat_embs = self.cat_embedding(cat_inputs)
