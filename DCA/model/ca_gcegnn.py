...
class CA_CombineGraph(Module):
    def __init__(self, conf, num_node, num_category, adj_all, num, logger):
    # conf is model_conf
        super(CA_CombineGraph, self).__init__()
    ...
    def forward(self, inputs, adj, mask_item, item):
        ...
        entity_vectors = [self.embedding(i) for i in item_neighbors] # i: batch_size * seq_len
    ...
    def _forward(self, data):
        ...
        inputs = inputs.long().to(self.device) # batch_size * seq_len
        cat_inputs = cat_inputs.long().to(self.device) # batch_size * seq_len
